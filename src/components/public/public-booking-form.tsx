...
export default function PublicBookingForm({ appointments }: PublicBookingFormProps) {
  ...
  const selectSuggestion = (suggestion: Suggestion) => {
    ...
    // In a real app, you would create a user account or use a guest ID.
    const customerId = `guest_${Date.now()}`;

    // Write to the public-facing collections
    addDocumentNonBlocking(collection(firestore, 'customers', customerId, 'appointments'), newAppointment);
    addDocumentNonBlocking(collection(firestore, 'stylists', suggestion.stylistId, 'appointments'), newAppointment);
    
    // Also write to the admin collection for dashboard visibility
    addDocumentNonBlocking(collection(firestore, 'admin_appointments'), newAppointment);
    ...
    <Card className="mx-auto w-full max-w-3xl">
        <Form {...form}>
            <form onSubmit={form.handleSubmit(findSuggestions)}>
                ...
                <CardDescription>
                    {step === 1 ? "Usa nuestro asistente inteligente para encontrar el momento perfecto." : "¡Excelentes opciones! Elige el horario que más te convenga."}
                </CardDescription>
                </CardHeader>
