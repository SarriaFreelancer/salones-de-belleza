...
export default function PublicBookingForm({
  ...
  // We pass false to useCollection to indicate it should NOT wait for a user.
  // This is because we need the appointments to calculate public availability.
  const { data: allAppointments } = useCollection<Appointment>(appointmentsCollection, false);
  ...
  const findAvailableSlots = (values: FormValues) => {
    if (!user) {
        toast({
            ...
            description: 'Por favor, inicia sesión o crea una cuenta para ver los horarios y agendar una cita.',
        });
        return;
    }
  ...
  const selectSlot = async (slot: Date) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          'No se pudo crear la cita. Por favor, revisa tus permisos e intenta de nuevo.',
        variant: 'destructive',
      });
    } finally {
      setIsCalculating(false);
        ...
        {step === 1 && (
            <>
                <CardHeader className="text-center">
                    ...
                    <CardDescription>Elige tu servicio y estilista para encontrar un horario disponible.</CardDescription>
                </CardHeader>
                <CardContent>
                     <Form {...form}>
                        <form
                        ...
                        />
                         <Button type="submit" disabled={isCalculating || isUserLoading} className="w-full" size="lg">
                            {isCalculating ? (
                                <>
                ...
                {!user && !isUserLoading && (
                    <CardFooter className="flex-col gap-4 text-center">
                        <p className="text-sm text-muted-foreground">
                            ¿Ya tienes una cuenta? Inicia sesión o regístrate para continuar.
                        </p>
                       <UserAuth />
                    </CardFooter>
                )}
          ...
          <>
            <CardHeader>
                <CardTitle className="font-headline text-3xl">Elige un Horario</CardTitle>
                 <div className="text-sm rounded-lg border bg-card text-card-foreground shadow-sm p-3 space-y-1">
                    <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                        <div><strong>Fecha:</strong> <Badge variant="outline">{form.getValues('preferredDate') ? format(form.getValues('preferredDate'), 'PPP', { locale: es }) : ''}</Badge></div>
                        ...
                        <div><strong>Duración:</strong> <Badge variant="outline">{selectedService?.duration} min</Badge></div>
                    </div>
                </div>
            </CardHeader>
            <CardContent>
